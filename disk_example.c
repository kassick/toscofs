...
int main(int argc, char *argv[])
{
    ...
    disk_t *d = disk_new("disk.data", disk_size);
    ...
    if (disk_write_sector(d, 10, sector)) {
    ...
    if(disk_write_sector(d, 0, sector)) {
    ...
    if (disk_write_sector(d, 1, sector)) {
    ...
    if (disk_read_sector(d, 1, sector)) {
        perror("Error reading back sector 0");
        exit(1);
    }
